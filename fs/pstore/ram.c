...
of_ramoops_platform_data(struct device *dev)
{
	...
	if (!of_property_read_u32(node, "android,ramoops-record-size", &val))
		pdata->record_size = val;
	if (!of_property_read_u32(node, "android,ramoops-console-size", &val))
		pdata->console_size = val;
	if (!of_property_read_u32(node, "android,ramoops-ftrace-size", &val))
		pdata->ftrace_size = val;
	if (!of_property_read_u32(node, "android,ramoops-pmsg-size",&val))
		pdata->pmsg_size = val;
	if (of_get_property(node, "android,ramoops-dump-oops", NULL))
		pdata->dump_oops = 1;
...
static void  ramoops_of_init(struct platform_device *pdev)
{
    ...
    u32 record = 0, dump_oops = 0,ftrace = 0;
    int ret;
   ...
   if (ret)
    ...
    if (of_get_property(np, "android,ramoops-dump-oops", NULL))
        dump_oops = 1;
